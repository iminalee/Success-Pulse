...
const deleteLedgerEntry = (logToDelete) => {
  ...
  const renderLab = () => {
    const missionMap = {
      1: "신체적 활력은 모든 변화의 엔진입니다. 이 단계에서는 수면의 질, 영양, 규칙적인 운동 등 나의 '생물학적 하드웨어'를 최적화해야 합니다. '하루 7시간 숙면'이나 '매일 30분 산책'처럼 에너지를 즉각적으로 높여줄 구체적인 행동 목표를 설정해 보세요.",
      2: "불안을 제거하고 심리적 안전 기지를 구축하는 단계입니다. 재정적 안정과 환경적 쾌적함이 핵심입니다. '비상금 확보'나 '주거 환경 개선'과 같이 외부 충격으로부터 나를 보호하고 평온함을 유지할 수 있는 환경적 토대를 만드는 목표를 세우는 것이 효과적입니다.",
      3: "건강한 관계 속에서 정서적 지지대를 형성하는 단계입니다. 고립감은 성장의 적입니다. 가족과의 깊은 대화, 진실한 친구 사귀기, 혹은 공통 가치를 지향하는 커뮤니티 활동 등 '나는 연결되어 있다'는 감각을 강화할 수 있는 관계 중심적 목표를 설정하세요.",
      4: "사회적 성취를 통해 스스로의 유능함을 확인하고 자부심을 확립하는 단계입니다. 전문 지식 습득이나 프로젝트 완수처럼 나의 가치를 실질적인 결과물로 증명하고, 스스로를 존경할 수 있게 만드는 도전적인 목표를 세워보세요.",
      5: "나의 잠재력을 완전히 꽃피워 'Apex BP'로서의 존재 의미를 완성하는 최종 단계입니다. 외부 보상보다는 내면의 부름에 응답하는 삶입니다. 창의적 활동이나 사회적 공헌 등 내가 진정으로 원하는 나의 모습에 도달하여 '살아있음의 전율'을 느끼는 궁극의 목표를 설정하십시오.",
    };
