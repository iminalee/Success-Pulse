...
const App = () => {
  ...
  // [수정 완료] 아이콘 업데이트: 5단계(명상), 4단계(트로피), 2단계(방패)
  ...
  const renderPhilosophy = () => (
    ...
    <div className="flex-grow w-full max-w-7xl mx-auto overflow-y-auto no-scrollbar pb-24 px-6 animate-fadeIn font-sans text-left">
      ...
      <div className="space-y-20">
        ...
        {/* [수정 완료] 용어집 한 줄 정렬 (flex-wrap items-baseline) */}
        <section className="mt-20 pt-12 border-t border-white/5 w-full">
          ...
          <div className="flex flex-col gap-3 w-full">
            <div className="p-4 bg-slate-900/20 rounded-2xl border border-white/5 hover:bg-slate-900/40 transition-all flex flex-wrap items-baseline gap-x-3"><span className="text-white font-bold text-base min-w-fit">Apex BP :</span><span className="text-slate-400 font-light text-sm">평행세계에서 모든 목표를 이룬 당신의 완성된 자아.</span></div>
            <div className="p-4 bg-slate-900/20 rounded-2xl border border-white/5 hover:bg-slate-900/40 transition-all flex flex-wrap items-baseline gap-x-3"><span className="text-white font-bold text-base min-w-fit">The Vessel :</span><span className="text-slate-400 font-light text-sm">마스터 자아의 의식을 현실에서 구현해내는 당신의 현재 육체.</span></div>
            <div className="p-4 bg-slate-900/20 rounded-2xl border border-white/5 hover:bg-slate-900/40 transition-all flex flex-wrap items-baseline gap-x-3"><span className="text-white font-bold text-base min-w-fit">Mental Bank :</span><span className="text-slate-400 font-light text-sm">행동의 가치가 복리로 적립되는 당신의 무의식 자산 계좌.</span></div>
            <div className="p-4 bg-slate-900/20 rounded-2xl border border-white/5 hover:bg-slate-900/40 transition-all flex flex-wrap items-baseline gap-x-3"><span className="text-white font-bold text-base min-w-fit">Magnitude :</span><span className="text-slate-400 font-light text-sm">현실을 변화시킨 에너지의 총량. 100% 도달 시 합일이 일어납니다.</span></div>
          </div>
        </section>
      </div>
    </div>
              ...
              <div className="flex-grow w-full">
                {user ? (
                  ...
                  <div className="flex flex-col gap-3">
                    {!isOtpSent ? (<><input value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="Name" className="bg-slate-900 border border-white/10 rounded-2xl p-4 text-white outline-none" /><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className="bg-slate-900 border border-white/10 rounded-2xl p-4 text-white outline-none" /><button onClick={handleLogin} disabled={loading} className="bg-blue-600 text-white font-bold py-4 rounded-2xl transition-all">{loading ? "전송 중..." : "인증번호 받기"}</button></>) : (<div className="space-y-3"><input type="text" value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="12345678" className="w-full bg-slate-900 border border-amber-500/50 rounded-2xl p-4 text-center text-2xl font-black text-amber-500 outline-none" /><button onClick={handleVerifyOtp} className="w-full bg-emerald-600 text-white font-bold py-4 rounded-2xl">확인</button><button onClick={() => setIsOtpSent(false)} className="w-full text-xs text-slate-500 py-2">다시 입력</button></div>)}
                  </div>
                )}
              </div>
            </div>
