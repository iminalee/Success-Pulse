...
import {
  Sparkles, Plus, Activity, Brain, BookOpen, Edit3, Save, Trash2, X, Home,
  ...
  PenTool, ShieldCheck, Info, TrendingUp, Star
} from "lucide-react";
...
const SensoryItem = ({ label, color, val }) => (
  <div className={`bg-[#1A202C]/40 p-5 rounded-[2rem] border-l-8 border-${color === "amber" ? "amber-600" : color === "emerald" ? "emerald-600" : "rose-600"} shadow-xl hover:bg-slate-900/60`}>
...
const App = () => {
  ...
  const [visions, setVisions] = useState({
    6: { title: "Apex Identity 확립", emoji: "👑", immersionScript: "", progressAsset: 0, events: [] },
    5: { title: "", emoji: "🧘", immersionScript: "", progressAsset: 0, events: [] },
    4: { title: "", emoji: "🏆", immersionScript: "", progressAsset: 0, events: [] },
    3: { title: "", emoji: "🤝", immersionScript: "", progressAsset: 0, events: [] },
    2: { title: "", emoji: "🛡️", immersionScript: "", progressAsset: 0, events: [] },
    1: { title: "", emoji: "🏃", immersionScript: "", progressAsset: 0, events: [] },
  });
    ...
    return (
      <div className="flex-grow w-full max-w-4xl mx-auto overflow-y-auto no-scrollbar pb-24 px-4 animate-fadeIn">
        ...
        <div className={`bg-[#0A0F1E] border-2 rounded-[3.5rem] p-8 md:p-16 relative overflow-hidden shadow-2xl ${isLocked ? "border-emerald-500/30" : "border-white/10"}`}>
          ...
          <div className="space-y-8 text-slate-300 leading-relaxed text-base md:text-lg text-justify relative z-10">
            <p className="border-l-2 border-amber-500/40 pl-6 font-sans">
              마스터 자아 <span className="text-white font-bold italic">Apex BP {userName}</span>는 현실 대리인 <span className="text-amber-400 font-bold">{userName} ver.0</span>의 본질적 도약을 위한 <span className="text-white font-black">‘존재적 상장(Existential IPO)’</span>을 승인하였다.
            </p>
            <p className="border-l-2 border-slate-700 pl-6">
              이에 <span className="text-slate-100 font-bold">{userName} ver.0</span>는 <input type="date" value={targetDate} disabled={isLocked} onChange={(e) => setTargetDate(e.target.value)} className="bg-slate-900 text-amber-500 border-none px-2 rounded font-bold" /> 까지 5단계 관문을 돌파한다. 대가로 Apex BP는 Mental Bank 잔액({currency}{fNum(mbBalance)})의 <span className="text-amber-500 font-black">25%</span>인 <span className="text-white bg-white/5 px-3 py-1 rounded-xl">{currency}{fNum(livingAllowance)}</span>을 즉시 수혈한다.
            </p>
            <p className="border-l-2 border-slate-700 pl-6">
              몰입이 증명될 때마다 시간당 <span className="text-emerald-400 font-black">{currency}{fNum(valueEventAmount)}</span>의 <span className="text-xs font-black text-emerald-500">Value Award</span>를 존재적 매출로 예치하며 동기화를 가속한다.
            </p>
            <p className="border-l-2 border-rose-500/50 pl-6 bg-rose-500/5 py-6 rounded-r-3xl italic">
              단, 3일 이상 무활동 시, 본 그릇(Vessel)과의 동기화를 해제하고 다른 평행 세계의 자아를 찾아 새로운 현신을 선택할 권리를 유지한다.
            </p>
          </div>
          <div className="mt-12 flex flex-col md:flex-row justify-between items-end gap-8">
            <div className="flex flex-col gap-4">
              ...
              {!isLocked && <button onClick={() => {if(signature===userName) setSignedDate(new Date()); else showToast("서명 불일치");}} className="bg-amber-600 px-8 py-3 rounded-xl font-black text-xs uppercase text-white shadow-lg">Execute</button>}
            </div>
            <div className="text-right text-amber-800/40 font-serif italic text-4xl">BP {userName} Authorized</div>
          </div>
        </div>
      </div>
    );
  ...
  const renderLab = () => {
    const missionMap = { 1: "생존의 기초. 변화를 위한 엔진을 가동합니다.", 2: "재정적 자립과 안전망을 구축하여 기반을 확보합니다.", 3: "건강한 관계 속에서 정서적 지지대를 구축합니다.", 4: "사회적 성취와 가치 증명을 통해 자아를 확립합니다.", 5: "타고난 잠재력을 발현하여 최고의 자아로 거듭납니다." };
    return (
              ...
              <div className="flex-grow w-full">
                {user ? (
                  ...
                  <div className="flex flex-col gap-3">
                    <input value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="Name" className="bg-slate-900 border border-white/10 rounded-2xl p-4 text-white outline-none" />
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className="bg-slate-900 border border-white/10 rounded-2xl p-4 text-white outline-none" />
                    <button onClick={handleLogin} className="bg-blue-600 text-white font-bold py-4 rounded-2xl transition-all">로그인</button>
                  </div>
                )}
              </div>
            </div>
            <div className="bg-[#2D3748]/40 p-10 rounded-[3.5rem] border border-white/5 shadow-xl">
              <div className="flex flex-col md:flex-row justify-between gap-4 mb-10">
                <div className="space-y-1">
                  ...
                  <p className="text-[12px] font-bold text-amber-500 italic bg-slate-900/50 px-3 py-1 rounded-full border border-white/5 inline-block">{activeLevel}단계: {levelMap[activeLevel]}</p>
                  <p className="text-[11px] text-slate-400 leading-relaxed mt-2 max-w-md">{missionMap[activeLevel]}</p>
                </div>
      ...
      <div className="relative w-full h-full flex-grow flex flex-col overflow-y-auto no-scrollbar pb-24">
        {!signedDate && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-slate-950/60 backdrop-blur-[6px] animate-fadeIn">
            <div className="bg-[#0A0F1E] border border-amber-500/30 p-10 rounded-[3rem] text-center shadow-2xl transform transition-all hover:scale-105">
              ...
              <p className="text-slate-400 text-sm leading-relaxed mb-8">서약서에 서명하면 모든 기능이 활성화됩니다.</p>
              <button onClick={() => setCurrentView("contract")} className="bg-amber-600 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl flex items-center gap-3 mx-auto active:scale-95 transition-all"><PenTool size={14} /> Sign Agreement</button>
            </div>
          </div>
        )}
        <div className={`flex flex-col md:flex-row items-center justify-center w-full h-full px-4 md:px-10 gap-8 ${!signedDate ? "opacity-40 blur-sm pointer-events-none" : "opacity-100"}`}>
