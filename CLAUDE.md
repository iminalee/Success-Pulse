...
* Supabase pulse\_data is the single profile store.
* ledger is the single source of truth for Mental Bank deposits.
* Apex AI must read the latest pulse\_data before responding.
* AI must not silently overwrite profile data. It may propose PROFILE\_PATCH, and the user must approve.
...
* Keep backward compatibility with existing fields: user\_name, annual\_income, target\_date, ledger, visions, bps\_traits, vak\_profile, tci\_profile, signature, signed\_date, apex\_conversation\_id.
...
3. My Lab manual values must save to the same pulse\_data profile.
4. Apex chat server must verify Supabase access token, load pulse\_data, and pass profile context to Dify.
